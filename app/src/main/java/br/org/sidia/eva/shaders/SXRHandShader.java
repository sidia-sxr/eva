 ...
 *
 * @<code> a_position    position vertex attribute
 ...
 * </code>
 */
public class SXRHandShader extends SXRShaderTemplate {
    public SXRHandShader(SXRContext sxrContext) {
        super("float3 u_color; float u_opacity; float u_time",
                "sampler2D u_texture",
                "float3 a_position; float2 a_texcoord", GLSLESVersion.VULKAN);
        Context context = sxrContext.getContext();
        setSegment("FragmentTemplate", TextFile.readTextFile(context, R.raw.tex_hand_frag));
        setSegment("VertexTemplate", TextFile.readTextFile(context, R.raw.tex_hand_vert));
    }

    protected void setMaterialDefaults(SXRShaderData material) {
        material.setVec3("u_color", 1, 1, 1);
        material.setFloat("u_opacity", 1f);
        material.setFloat("u_time", 0f);
    }
}
